...
if parser.read(configPath)[-1] == 'config.ini':
	...
	if _:
		...
		# reformat it asap
		permFileName = parser['APP_CONFIG'].get('CameraName')+'_'+str(currentTime).replace(' ', '_').replace(':', '_')+'.jpg'
		cv2.imwrite(permFileName, image)
		blob = bucket.blob(permFileName)
		blob.upload_from_filename(permFileName)
		tempIntruderLogDocument = intruder_collection.document(parser['APP_CONFIG'].get('CameraName')+' '+str(currentTime))
